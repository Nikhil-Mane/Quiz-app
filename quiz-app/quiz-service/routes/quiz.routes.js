...
router.post('/:quizId/submit', auth, async (req, res) => {
  try {
    ...
    const previousAttempts = await Result.countDocuments({ quiz_id: quizId, user_id });
...
router.get('/:id/results/:userId', auth, async (req, res) => {
  try {
    const results = await Result.find({ quiz_id: req.params.id, user_id: req.params.userId })
      .sort({ attempt: 1 });

    if (!results.length) return res.status(404).json({ message: 'No attempts found' });
